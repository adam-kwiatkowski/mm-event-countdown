  ...
  async socketNotificationReceived(notification, payload) {
    switch (notification) {
      case "EC_ADD_CALENDAR":
        ...
        break;
      case "EC_FETCH_CALENDAR":
        const key = payload.id + payload.url;
        if (typeof this.fetchers[key] === "undefined") {
          Log.error("[EC] Calendar Error. No fetcher exists with key: ", key);
          this.sendSocketNotification("EC_CALENDAR_ERROR", { error_type: "MODULE_ERROR_UNSPECIFIED" });
          return;
        }
    ...
    // if (notification === "EC_ADD_CALENDAR") {
		// 	this.createFetcher(payload.url, payload.fetchInterval, payload.excludedEvents, payload.maximumEntries, payload.maximumNumberOfDays, payload.auth, payload.broadcastPastEvents, payload.selfSignedCert, payload.id);
		// } else if (notification === "EC_FETCH_CALENDAR") {
		// 	const key = payload.id + payload.url;
		// 	if (typeof this.fetchers[key] === "undefined") {
  ...
  /**
	...
	createFetcher (url, fetchInterval, excludedEvents, maximumEntries, maximumNumberOfDays, auth, broadcastPastEvents, selfSignedCert, identifier) {
		try {
			...
			Log.error("[EC] Calendar Error. Malformed calendar url: ", url, error);
			this.sendSocketNotification("EC_CALENDAR_ERROR", { error_type: "MODULE_ERROR_MALFORMED_URL" });
			return;
		}
		...
		if (typeof this.fetchers[identifier + url] === "undefined") {
			if (fetchInterval < 60000) {
				Log.warn(`[EC] fetchInterval for url ${url} must be >= 60000`);
				fetchIntervalCorrected = 60000;
			}
			Log.log(`[EC] Create new calendarfetcher for url: ${url} - Interval: ${fetchIntervalCorrected || fetchInterval}`);
			...
			fetcher.onError((fetcher, error) => {
				Log.error("[EC] Calendar Error. Could not fetch calendar: ", fetcher.url(), error);
				let error_type = NodeHelper.checkFetchError(error);
				this.sendSocketNotification("EC_CALENDAR_ERROR", {
					id: identifier,
					error_type
				});
			});
			...
			Log.log(`[EC] Use existing calendarfetcher for url: ${url}`);
	...
	broadcastEvents (fetcher, identifier) {
		this.sendSocketNotification("EC_CALENDAR_EVENTS", {
			id: identifier,
			url: fetcher.url(),
